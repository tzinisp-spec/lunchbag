  ...
  useEffect(() => {
    ...
    async function poll() {
      try {
        ...
        // Photoshoot report ready
        if (!prev.sprint_ready && s.sprint_ready) {
          addToast('info', 'Photoshoot report ready', {
            action: { label: 'See report', onClick: () => navigate('/photoshoot-report') },
          })
        }
        // Content plan report ready
        if (!prev.sprint_ready_p2 && s.sprint_ready_p2) {
          addToast('info', 'Content planning report ready', {
            action: { label: 'See report', onClick: () => navigate('/content-plan-report') },
          })
        }
  ...
  return (
    <div className="flex h-screen bg-[var(--c-page)] text-[var(--c-text-1)] overflow-hidden">
      ...
      <div className="hidden md:flex w-16 bg-[var(--c-sidebar)] border-r border-[var(--c-border-subtle)] flex-col items-center py-4 gap-3 shrink-0">
        <div className="w-10 h-10 bg-[var(--c-icon-box)] rounded-lg flex items-center justify-center select-none" title="COMAP">
        ...
        <div className="md:hidden sticky top-0 z-10 bg-[var(--c-sidebar)] border-b border-[var(--c-border)] px-4 py-3 flex items-center gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2 flex-1">
            <div className="w-6 h-6 bg-[var(--c-icon-box)] rounded flex items-center justify-center">
              <span className="text-[var(--c-text-1)] font-bold text-[9px] leading-none">C</span>
            </div>
            <span className="text-[var(--c-text-1)] font-medium text-sm">COMAP</span>
          </div>
