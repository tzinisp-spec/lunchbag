    ...
    <div className="p-8 max-w-4xl">
      ...
      <div className={`rounded-xl border ${c.border} ${c.bg} px-8 py-7 mb-8 flex items-start gap-6`}>
        ...
        <div>
          ...
          <h1 className="text-3xl text-white font-semibold mb-2">{agent.name}</h1>
          <p className="text-gray-400 text-sm italic">"{agent.tagline}"</p>
        </div>
      </div>
      ...
      <Section title="Goal">
        <div className={`rounded-lg border ${c.border} ${c.bg} px-6 py-5`}>
          <p className="text-gray-200 text-sm leading-relaxed">{agent.goal}</p>
        </div>
      </Section>
      ...
      <Section title="What they do">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {agent.what_they_do.map((item, i) => (
            <div key={i} className="bg-gray-900 border border-gray-800 rounded-lg px-5 py-4 flex gap-4">
      ...
      <Section title="Tasks">
        <div className="space-y-2">
          {agent.tasks.map((task, i) => (
            <div key={i} className="bg-gray-900 border border-gray-800 rounded-lg px-5 py-4 flex items-start gap-4">
              ...
              <div>
                ...
                <div className="text-gray-500 text-xs mt-0.5 leading-relaxed">{task.description}</div>
              </div>
            </div>
          ))}
        </div>
              ...
              return (
                <div key={a.id} className="flex items-center gap-1 shrink-0">
                  ...
                  >
                    ...
                    <span className={`text-xs font-medium text-center leading-tight ${isCurrent ? 'text-white' : 'text-gray-500'}`} style={{ maxWidth: '72px' }}>
      ...
      <div className="flex gap-3 mt-2">
        {prev ? (
          <Link
            to={`/agents/${prev.id}`}
            className="flex-1 bg-gray-900 border border-gray-800 rounded-lg px-5 py-4 flex items-center gap-3 hover:bg-gray-800/50 transition-colors"
          >
            <ArrowLeft size={15} className="text-gray-500 shrink-0" />
            <div>
              <div className="text-gray-500 text-xs">Previous agent</div>
              <div className="text-white text-sm font-medium">{prev.name}</div>
            </div>
          </Link>
        ...
        {next ? (
          <Link
            to={`/agents/${next.id}`}
            className="flex-1 bg-gray-900 border border-gray-800 rounded-lg px-5 py-4 flex items-center justify-end gap-3 hover:bg-gray-800/50 transition-colors"
          >
            <div className="text-right">
              <div className="text-gray-500 text-xs">Next agent</div>
              <div className="text-white text-sm font-medium">{next.name}</div>
            </div>
